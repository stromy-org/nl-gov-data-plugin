...
### Image Sizing — Aspect Ratio Preservation

**CRITICAL**: Never set both `width` and `height` on an `<img>` tag to values that create a different aspect ratio than the source image. This causes visible stretching in the PowerPoint output.

**Safe patterns:**
- Set only width: `<img style="width: 240pt;">` — height auto-calculated
- Set only height: `<img style="height: 405pt;">` — width auto-calculated
- Set both BUT matching source ratio: Pre-crop with Sharp first, then use exact dimensions

**Unsafe pattern (causes stretching):**
```html
<!-- BAD: 16:9 photo forced into ~3:5 box -->
<img src="bridge-16x9.jpg" style="width: 240pt; height: 405pt;">
```

**For split-layout slides**: Always pre-crop source images to the target aspect ratio using Sharp's `extract()` + `resize()`, then reference the cropped version in HTML. See [design-guide.md](references/design-guide.md) for the crop helper function and aspect ratio targets.
